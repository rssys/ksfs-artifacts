...
res_err_out:
		...
		fuse_zc_memcpy(b, val + pos, count);
		ntfs_attr_put_search_ctx(ctx);
		return count;
	}
	total = total2 = 0;
	...
	if (pos + count > max_init) {
		if (pos >= max_init) {
			//fuse_zc_memset(b, 0, count);
			return count;
		}
		total2 = pos + count - max_init;
		count -= total2;
		//fuse_zc_memset((u8*)b + count, 0, total2);
	}
		/*
	...
	if (na->ni->vol->efs_raw && 
		...
		if (pos+count == max_init) {
			if (count == 1) {
				...
				fuse_zc_memcpy((u8*)b+count-1, &tmp, sizeof(tmp));
				//*((u8*)b+count-1) = (u8)(efs_padding_length >> 8);
				count--;
				total2++;
			} else {
				le16 tmp = cpu_to_le16(efs_padding_length);
				fuse_zc_memcpy((u8*)b+count-2, &tmp, sizeof(tmp));
								...
								const void *b)
{
	...
	if (!NAttrNonResident(na)) {
		...
		fuse_zc_memcpy(val + pos, b, count);
