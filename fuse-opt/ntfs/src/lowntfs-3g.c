			...
			struct fuse_file_info *fi __attribute__((unused)))
{
	...
	ntfs_attr *na = NULL;
	int res;
	char *buf = (char*)FUSE_ZC_BUF_BASE;
	s64 total = 0;
	s64 max_read;

	fuse_set_curr_req(req);
...
static void ntfs_fuse_write_buf(fuse_req_t req, fuse_ino_t ino,
			struct fuse_bufvec *mbuf, off_t offset,
			struct fuse_file_info *fi __attribute__((unused)))
{
	ntfs_inode *ni = NULL;
	ntfs_attr *na = NULL;
	int res, total = 0;
	const char *buf = FUSE_ZC_BUF_BASE;
...
static void remove_unknown_options(char *parsed_options)
{
	char *p = strstr(parsed_options, "nonempty");
	if (p) {
		if (p[8] == ',') {
	...
	p = strstr(parsed_options, "relatime");
	if (p) {
		if (p[8] == ',') {
			memmove(p, p + 9, strlen(p + 9) + 1);
		} else {
...
static struct fuse_session *mount_fuse(char *parsed_options)
{
	...
	if (fuse_opt_add_arg(&args, EXEC_NAME) == -1)
		goto err;
	if (ctx->debug)
		if (ntfs_strappend(&parsed_options, ",debug") == -1)
			goto err;
	if (fuse_opt_add_arg(&args, "-o") == -1)
		goto err;
	remove_unknown_options(parsed_options);
	if (fuse_opt_add_arg(&args, parsed_options) == -1)
		goto err;
