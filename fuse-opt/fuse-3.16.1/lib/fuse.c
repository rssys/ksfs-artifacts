...
static __thread int curr_devfd;
static __thread uint64_t curr_unique;

void fuse_set_curr_req(struct fuse_req *req)
{
	curr_devfd = req->ch ? req->ch->fd : req->se->fd;
	curr_unique = req->unique;
}

void *fuse_zc_memcpy(void *dst, const void *src, size_t count)
{
	struct fuse_dev_ioctl_zerocopy cmd = {
		.cmd = FUSE_DEV_IOC_ZEROCOPY_MEMCPY,
		.unique = curr_unique,
	};
	if (!fuse_is_zc_addr(dst) && !fuse_is_zc_addr(src))
		return memcpy(dst, src, count);
	cmd.memcpy_args.dst = (uint64_t)(fuse_is_zc_addr(dst) ? (dst - FUSE_ZC_BUF_BASE) : dst);
	cmd.memcpy_args.src = (uint64_t)(fuse_is_zc_addr(src) ? (src - FUSE_ZC_BUF_BASE) : src);
	cmd.memcpy_args.count = count;
	if (ioctl(curr_devfd, FUSE_DEV_IOC_ZEROCOPY, &cmd))
		abort();
	return dst;
}

void *fuse_zc_memset(void *dst, int ch, size_t count)
{
	struct fuse_dev_ioctl_zerocopy cmd = {
		.cmd = FUSE_DEV_IOC_ZEROCOPY_MEMSET,
		.unique = curr_unique
	};
	if (!fuse_is_zc_addr(dst))
		return memset(dst, ch, count);
	cmd.memset_args.dst = (uint64_t)(dst - FUSE_ZC_BUF_BASE);
	cmd.memset_args.ch = ch;
	cmd.memset_args.count = count;
	if (ioctl(curr_devfd, FUSE_DEV_IOC_ZEROCOPY, &cmd))
		abort();
	return dst;
}

ssize_t fuse_azc_pread(int fd, void *addr, size_t count, off_t offset)
{
	struct fuse_dev_ioctl_zerocopy cmd = {
		.cmd = FUSE_DEV_IOC_ZEROCOPY_PREAD,
		.unique = curr_unique
	};
	if (!fuse_is_zc_addr(addr))
		return pread(fd, addr, count, offset);
	cmd.file_args.fd = fd;
	cmd.file_args.buf = (uint64_t)(addr - FUSE_ZC_BUF_BASE);
	cmd.file_args.count = count;
	cmd.file_args.off = offset;
	return ioctl(curr_devfd, FUSE_DEV_IOC_ZEROCOPY, &cmd);
}

ssize_t fuse_azc_pwrite(int fd, const void *addr, size_t count, off_t offset)
{
	struct fuse_dev_ioctl_zerocopy cmd = {
		.cmd = FUSE_DEV_IOC_ZEROCOPY_PWRITE,
		.unique = curr_unique
	};
	if (!fuse_is_zc_addr(addr))
		return pwrite(fd, addr, count, offset);
	cmd.file_args.fd = fd;
	cmd.file_args.buf = (uint64_t)(addr - FUSE_ZC_BUF_BASE);
	cmd.file_args.count = count;
	cmd.file_args.off = offset;
