...
// /// A wrapper around the kernel `struct file` type.
def_kernel_obj_type!(RsFile);
...
fn is_err(ptr: *const c_void) -> bool {
    ...
    uptr >= (!0usize - MAX_ERRNO)
}

#[inline]
fn ptr_err(ptr: *const c_void) -> i32 {
    let uptr = ptr as isize;
    uptr as i32
}
...
impl RsBlockDevice {
    pub fn new(name: &str) -> Result<Self, i32> {
        unsafe {
            let bdev = blkdev_get_by_path(name.as_ptr() as *const c_char,
                FMODE_READ | FMODE_WRITE, bento_get_blkdev_holder());
            if is_err(bdev) {
                return Err(-ptr_err(bdev));
            }
            Ok(Self::from_raw(bdev))
        }
    }
    ...
    pub fn get_size(&self) -> u64 {
        unsafe {
            bento_blkdev_get_size(self.get_raw())
        }
    }

    pub fn sync(&self) -> Result<(), i32> {
        let res = unsafe {
            sync_blockdev(self.get_raw())
        };
        if res < 0 {
            Err(-res)
        } else {
...
impl RsFile {
    pub fn new(name: &str, flags: i32, mode: u16) -> Result<RsFile, i32> {
        unsafe {
            let filp = filp_open(name.as_ptr() as *const c_char,
                flags, mode);
            if is_err(filp) {
                return Err(-ptr_err(filp));
            }
            Ok(Self::from_raw(filp))
        }
    }

    pub fn pread(&self, buf: &mut [u8], offset: u64) -> Result<usize, i32> {
        let buf_len = buf.len();
        let buf_ptr = buf.as_mut_ptr();
        let mut pos: i64 = offset.try_into().unwrap();
        let pos_ptr: *mut i64 = &mut pos;
        let res = unsafe {
            kernel_read(self.get_raw(), buf_ptr as *mut c_void,
                buf_len, pos_ptr)
        };
        if res < 0 {
            Err(res.try_into().unwrap())
        } else {
            Ok(res.try_into().unwrap())
        }
    }

    pub fn pwrite(&self, buf: &[u8], offset: u64) -> Result<usize, i32> {
        let buf_len = buf.len();
        let buf_ptr = buf.as_ptr();
        let mut pos: i64 = offset.try_into().unwrap();
        let pos_ptr: *mut i64 = &mut pos;
        let res = unsafe {
            kernel_write(self.get_raw(), buf_ptr as *const c_void,
                buf_len, pos_ptr)
        };
        if res < 0 {
            Err(res.try_into().unwrap())
    ...
    pub fn fsync(&self, datasync: bool) -> Result<(), i32> {
        let res = unsafe {
            vfs_fsync(self.get_raw(), datasync as i32)
        };
        if res < 0 {
            Err(res.try_into().unwrap())
        } else {
    ...
    pub fn close(&mut self) -> i32 {
        unsafe {
            filp_close(self.get_raw(), 0 as *const c_void)
        }
    }

    pub fn lseek(&self, offset: i64, whence: i32) -> Result<i64, i32> {
        let res = unsafe {
            vfs_llseek(self.get_raw(), offset, whence)
        };
        if res < 0 {
            Err(res.try_into().unwrap())
        } else {
...
unsafe impl Send for RsFile {}
unsafe impl Sync for RsFile{}
