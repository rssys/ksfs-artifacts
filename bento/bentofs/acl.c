...
struct posix_acl *bento_get_acl(struct inode *inode, int type, bool rcu)
{
...
int bento_set_acl(struct user_namespace *ns, struct inode *inode, struct posix_acl *acl, int type)
{
	struct bento_conn *fc = get_bento_conn(inode);
	const char *name;
	int ret;
