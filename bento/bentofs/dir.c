 ...
 * 'mknod' + 'open' requests.
 */
static int bento_create_open(struct user_namespace *ns, struct inode *dir, struct dentry *entry,
...
static int bento_mknod(struct user_namespace *ns, struct inode *, struct dentry *, umode_t, dev_t);
			    ...
			    umode_t mode)
{
	...
	err = bento_create_open(NULL, dir, entry, file, flags, mode);
	if (err == -ENOSYS) {
		fc->no_create = 1;
		goto mknod;
	}
...
mknod:
	err = bento_mknod(NULL, dir, entry, mode, 0);
	if (err)
		goto out_dput;
no_open:
...
static int bento_mknod(struct user_namespace *ns, struct inode *dir, struct dentry *entry, umode_t mode,
		      dev_t rdev)
{
...
static int bento_create(struct user_namespace *ns, struct inode *dir, struct dentry *entry, umode_t mode,
		       bool excl)
{
	return bento_mknod(ns, dir, entry, mode, 0);
}

static int bento_mkdir(struct user_namespace *ns, struct inode *dir, struct dentry *entry, umode_t mode)
{
...
static int bento_symlink(struct user_namespace *ns, struct inode *dir, struct dentry *entry,
			const char *link)
{
	struct bento_conn *fc = get_bento_conn(dir);
...
static int bento_rename2(struct user_namespace *ns, struct inode *olddir, struct dentry *oldent,
			struct inode *newdir, struct dentry *newent,
			unsigned int flags)
{
	struct bento_conn *fc = get_bento_conn(olddir);
...
static int bento_update_get_attr(struct user_namespace *ns, struct inode *inode, struct file *file,
				struct kstat *stat)
{
	struct bento_inode *fi = get_bento_inode(inode);
	int err = 0;
...
{
	return bento_update_get_attr(current_user_ns(), inode, file, NULL);
}
 ...
 */
static int bento_permission(struct user_namespace *ns, struct inode *inode, int mask)
{
	struct bento_conn *fc = get_bento_conn(inode);
...
static int bento_setattr(struct user_namespace *ns, struct dentry *entry, struct iattr *attr)
{
	struct inode *inode = d_inode(entry);
	struct bento_conn *fc = get_bento_conn(inode);
...
static int bento_getattr(struct user_namespace *ns, const struct path *path, struct kstat *stat,
			u32 request_mask, unsigned int flags)
{
	...
	return bento_update_get_attr(ns, inode, NULL, stat);
}
