...
__attribute__((import_module("env"), import_name("task_has_gid")))
extern int task_has_gid(pid_t pid, gid_t gid);
...
static BOOL groupmember(struct SECURITY_CONTEXT *scx, uid_t uid, gid_t gid)
{
	...
	if (scx->vol->secure_flags & (1 << SECURITY_STATICGRPS))
		...
		ismember = task_has_gid(scx->tid, gid);
	return (ismember);
}
...
inline static int getuid()
{
	return 0;
}

inline static int getgid()
{
	return 0;
}
