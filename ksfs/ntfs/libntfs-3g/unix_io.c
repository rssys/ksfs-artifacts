...
static int ntfs_device_unix_io_open_fd(struct ntfs_device *dev, int flags)
{
	struct flock flk;
	struct stat sbuf;
	int err;
	...
	*(int*)dev->d_private = atoi(dev->d_name);
...
{
	s64 res;
	if (fuse_wasm_is_zc_addr(buf)) 
		res = fuse_wasm_azc_pread(DEV_FD(dev), buf, count, offset);
	else
		res = pread(DEV_FD(dev), buf, count, offset);
	return res;
}
		...
		s64 count, s64 offset)
{
	...
	NDevSetDirty(dev);
	if (fuse_wasm_is_zc_addr(buf))
		res = fuse_wasm_azc_pwrite(DEV_FD(dev), buf, count, offset);
	else
		res = pwrite(DEV_FD(dev), buf, count, offset);
	return res;
}
...
struct ntfs_device_operations ntfs_device_unix_io_ops = {
	.open		= ntfs_device_unix_io_open_fd,
