...
res_err_out:
		...
		fuse_wasm_zc_memcpy(b, val + pos, count);
		ntfs_attr_put_search_ctx(ctx);
		return count;
	}
	total = total2 = 0;
	...
	if (pos + count > max_init) {
		if (pos >= max_init) {
			//fuse_wasm_zc_memset(b, 0, count);
			return count;
		}
		total2 = pos + count - max_init;
		count -= total2;
		//fuse_wasm_zc_memset((u8*)b + count, 0, total2);
	}
		/*
	...
	if (na->ni->vol->efs_raw && 
		...
		if (pos+count == max_init) {
			if (count == 1) {
				u8 tmp = efs_padding_length >> 8;
				fuse_wasm_zc_memcpy((u8*)b+count-1, &tmp, sizeof(tmp));
				//*((u8*)b+count-1) = (u8)(efs_padding_length >> 8);
				count--;
				total2++;
			} else {
				le16 tmp = cpu_to_le16(efs_padding_length);
				fuse_wasm_zc_memcpy((u8*)b+count-2, &tmp, sizeof(tmp));
				//*(le16*)((u8*)b+count-2) = cpu_to_le16(efs_padding_length);
				count -= 2;
				total2 +=2;
			}
		} else {
			u8 tmp = efs_padding_length & 0xff;
			fuse_wasm_zc_memcpy((u8*)b+count-1, &tmp, sizeof(tmp));
	...
	for (; count; rl++, ofs = 0) {
		...
		if (rl->lcn < (LCN)0) {
			...
			fuse_wasm_zc_memset(b, 0, to_read);
								...
								const void *b)
{
	...
	if (!NAttrNonResident(na)) {
		...
		fuse_wasm_zc_memcpy(val + pos, b, count);
...
retry:
		...
		if (!NVolReadOnly(vol)) {
			...
			if (rounding && ((wend == na->initialized_size) ||
				...
				fuse_wasm_zc_memcpy(cb, b, to_write);
