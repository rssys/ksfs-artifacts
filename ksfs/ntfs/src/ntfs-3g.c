...
static void *ntfs_init(struct fuse_conn_info *conn, struct fuse_config *config)
{
#if defined(__APPLE__) || defined(__DARWIN__)
...
static int ntfs_fuse_getattr(const char *org_path, struct stat *stbuf, struct fuse_file_info *ffi)
{
	int res = 0;
	ntfs_inode *ni;
	ntfs_attr *na;
...
static int ntfs_fuse_truncate(const char *org_path, off_t size, struct fuse_file_info *fi)
{
	return ntfs_fuse_trunc(org_path, size, !fi);
}
...
static int mount_fuse(char *parsed_options)
{
	struct fuse *fh = NULL;
	...
	return fuse_main(args.argc, args.argv, &ntfs_3g_ops, NULL);
err:
	return 1;
}
...
int main(int argc, char *argv[])
{
	...
	if (mount_fuse(parsed_options) == 0)
		return 0;
