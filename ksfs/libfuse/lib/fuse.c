		     ...
		     struct fuse_file_info *fi)
{
	...
	if (fs->op.read || fs->op.read_buf) {
		...
		} else if (fuse_wasm_flags & FUSE_WASM_READ_ZERO_COPY) {
			return fs->op.read(path, (void *)FUSE_WASM_ZC_BUF_BASE, size, off, fi);
		} else {
...
static struct fuse_context_i *fuse_get_context_internal(void)
{
	...
	static struct fuse_context_i fuse_context_i;
	return &fuse_context_i;
}
			  ...
			  off_t off, struct fuse_file_info *fi)
{
	...
	if (fuse_wasm_flags & FUSE_WASM_READ_ZERO_COPY) {
		if (res < 0)
			reply_err(req, res);
		else
			fuse_reply_buf(req, (void *)FUSE_WASM_ZC_BUF_BASE, res);
	} else  {
		if (res == 0)
