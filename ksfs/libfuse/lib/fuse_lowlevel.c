...
static int send_reply_iov(fuse_req_t req, struct iovec *iov,
			  int count)
{
	int res = fuse_wasm_send_reply_iov(iov, count);
	//fuse_free_req(req);
	return res;
}
...
static int send_reply(fuse_req_t req, int error, const void *arg,
		      size_t argsize)
{
	int res = fuse_wasm_send_reply(error, arg, argsize);
	//fuse_free_req(req);
	return res;
}
...
{
	return send_reply_iov(req, (struct iovec *)iov, count);
}
		    ...
		    enum fuse_buf_copy_flags flags)
{
	...
	(void)req;
	(void)flags;
	res = fuse_wasm_send_reply_buf(bufv);
	//fuse_free_req(req);
	return res;
}
			   ...
			   const struct iovec *out_iov, size_t out_count)
{
	...
	res = send_reply_iov(req, iov, count);
out:
	free(in_fiov);
	free(out_fiov);
...
int fuse_reply_ioctl(fuse_req_t req, int result, const void *buf, size_t size)
{
	...
	return send_reply_iov(req, iov, count);
}
			 ...
			 int count)
{
	...
	memset(&arg, 0, sizeof(arg));
	arg.result = result;
	padded_iov[0].iov_base = &arg;
	padded_iov[0].iov_len = sizeof(arg);

	memcpy(&padded_iov[1], iov, count * sizeof(struct iovec));

	res = send_reply_iov(req, padded_iov, count + 2);
	free(padded_iov);
			   ...
			   struct iovec *iov, int count)
{
	...
	return fuse_wasm_send_notify_iov(notify_code, iov, count);
}
...
int fuse_lowlevel_notify_poll(struct fuse_pollhandle *ph)
{
	if (ph != NULL) {
		...
		return fuse_wasm_send_notify(FUSE_NOTIFY_POLL, &outarg, sizeof(outarg));
	} else {
		return 0;
	}
}
				     ...
				     off_t off, off_t len)
{
	...
	return fuse_wasm_send_notify(FUSE_NOTIFY_INVAL_INODE, &outarg, sizeof(outarg));
}
							...
							enum fuse_notify_entry_flags flags)
{
	...
	iov[0].iov_base = &outarg;
	iov[0].iov_len = sizeof(outarg);
	iov[1].iov_base = (void *)name;
	iov[1].iov_len = namelen + 1;

	return send_notify_iov(se, FUSE_NOTIFY_INVAL_ENTRY, iov, 2);
}
				...
				const char *name, size_t namelen)
{
	...
	return send_notify_iov(se, FUSE_NOTIFY_DELETE, iov, 2);
}
			       ...
			       enum fuse_buf_copy_flags flags)
{
	...
	res = fuse_wasm_send_notify_buf(FUSE_NOTIFY_STORE, &outarg, sizeof(outarg), bufv);
	if (res > 0)
		res = -res;
				  ...
				  size_t size, off_t offset, void *cookie)
{
	...
	err = fuse_wasm_send_notify(FUSE_NOTIFY_RETRIEVE, &outarg, sizeof(outarg));
	if (err) {
		pthread_mutex_lock(&se->lock);
...
__attribute__((export_name("fuse_wasm_process_msg")))
void fuse_wasm_process_msg(struct fuse_session *se, void *mem, size_t size,
			   void *buf_mem, size_t buf_size)
{
	struct fuse_in_header *in = mem;
	const void *inarg;
	struct fuse_req req = {
		.se = se,
		.ctr = 1
	...
	if (se->debug) {
		fuse_log(FUSE_LOG_DEBUG,
			...
			(unsigned long long) in->nodeid, size + buf_size, in->pid);
	}
