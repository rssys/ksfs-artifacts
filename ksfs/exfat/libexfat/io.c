...
struct exfat_dev* exfat_open_fd(int fd, enum exfat_mode mode)
{
	...
	}
	{
		...
		if (dev->size <= 0)
		{
			close(dev->fd);
			free(dev);
			exfat_error("failed to get size of '%d'", fd);
			return NULL;
		}
		if (exfat_seek(dev, 0, SEEK_SET) == -1)
		{
			close(dev->fd);
			free(dev);
			exfat_error("failed to seek to the beginning of '%d'", fd);
			return NULL;
		}
	}
...
#else
	if (fuse_wasm_is_zc_addr(buffer))
		return fuse_wasm_azc_pread(dev->fd, buffer, size, offset);
	else
...
#else
	if (fuse_wasm_is_zc_addr(buffer))
		return fuse_wasm_azc_pwrite(dev->fd, buffer, size, offset);
	else
		...
		void* buffer, size_t size, off_t offset)
{
	...
	if (uoffset + size > node->valid_size)
	{
		...
		fuse_wasm_zc_memset(buffer + bytes, 0,
