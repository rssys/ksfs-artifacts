...
static void reset_cache(struct exfat* ef, struct exfat_node* node)
{
	...
	if (!node->references && node->is_unlinked)
...
static int create_at(struct exfat* ef, struct exfat_node* dir,
		const char* name, uint16_t attrib)
{
	struct exfat_node* existing;
	off_t offset = -1;
	le16_t name16[EXFAT_NAME_MAX + 1] = {};
	int rc;

	rc = exfat_lookup_name(ef, dir, &existing, name);
	if (rc == 0)
	{
		exfat_put_node(ef, existing);
		return -EEXIST;
	...
	rc = exfat_utf8_to_utf16(name16, name, EXFAT_NAME_MAX + 1, strlen(name) + 1);

	if (rc != 0)
		return rc;

	rc = find_slot(ef, dir, &offset,
			2 + DIV_ROUND_UP(exfat_utf16_length(name16), EXFAT_ENAME_MAX));
	if (rc != 0)
		return rc;
	rc = commit_entry(ef, dir, name16, offset, attrib);
	if (rc != 0)
		return rc;
	exfat_update_mtime(dir);
	rc = exfat_flush_node(ef, dir);
...
int exfat_mknod_at(struct exfat* ef, struct exfat_node* dir, const char* name)
{
	return create_at(ef, dir, name, EXFAT_ATTRIB_ARCH);
}
...
int exfat_mkdir_at(struct exfat* ef, struct exfat_node* dir, const char* name)
{
	int rc;
	struct exfat_node* node;

	rc = create_at(ef, dir, name, EXFAT_ATTRIB_DIR);
	if (rc != 0)
		return rc;
	rc = exfat_lookup_name(ef, dir, &node, name);
	if (rc != 0)
		return 0;
	/* directories always have at least one cluster */
	rc = exfat_truncate(ef, node, CLUSTER_SIZE(*ef->sb), true);
...
int exfat_rename_at(struct exfat* ef, struct exfat_node* old_parent, const char* old_name,
	struct exfat_node *new_parent, const char* new_name)
{
	struct exfat_node* node;
	struct exfat_node* existing = NULL;
	struct exfat_node* dir = new_parent;
	off_t offset = -1;
	le16_t name[EXFAT_NAME_MAX + 1];
	int rc;

	rc = exfat_utf8_to_utf16(name, new_name, EXFAT_NAME_MAX + 1, strlen(new_name) + 1);

	if (rc != 0)
		return rc;

	rc = exfat_lookup_name(ef, old_parent, &node, old_name);
	if (rc != 0)
		return rc;

	rc = exfat_lookup_name(ef, new_parent, &existing, new_name);
	if (rc == -ENOENT)
	{
		existing = NULL;
	}
