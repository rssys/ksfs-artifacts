...
static void fuse_exfat_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info* fi)
{
	...
	exfat_debug("[%s] %s flags %#x%s%s%s%s%s", __func__, path, fi->flags,
			fi->flags & LINUX_O_RDONLY ? " O_RDONLY" : "",
			fi->flags & LINUX_O_WRONLY ? " O_WRONLY" : "",
			fi->flags & LINUX_O_RDWR   ? " O_RDWR"   : "",
			fi->flags & LINUX_O_APPEND ? " O_APPEND" : "",
			fi->flags & LINUX_O_TRUNC  ? " O_TRUNC"  : "");

	if (fi->flags & LINUX_O_TRUNC)
	{
...
{
	struct exfat_node* node = get_node_from_ino(ino);
	char* buf = (char*)FUSE_WASM_ZC_BUF_BASE;
	ssize_t rc;
...
static void fuse_exfat_write_buf(fuse_req_t req, fuse_ino_t ino,
		struct fuse_bufvec* mbuf, off_t offset, struct fuse_file_info* fi)
{
	struct exfat_node* node = get_node_from_ino(ino);
	ssize_t rc;
	const char* buf = mbuf->buf->mem;
	size_t size = mbuf->buf->size;
...
static struct fuse_lowlevel_ops fuse_exfat_ll_ops =
{
	...
	.write_buf	= fuse_exfat_write_buf,
...
static int fuse_exfat_main(char* mount_options, char* mount_point, int foreground)
{
	...
	if (!se)
		goto err_se;
	fuse_wasm_flags = FUSE_WASM_READ_ZERO_COPY | FUSE_WASM_WRITE_ZERO_COPY
			| FUSE_WASM_READ_ASYNC | FUSE_WASM_WRITE_ASYNC;
	fuse_wasm_set_session(se);
	return 0;
