...
static noinline void trap(void) 
{
...
//{
//    unsigned long bottom = current_top_of_stack() - THREAD_SIZE;
//    register unsigned long rsp asm ("rsp");
//    if (current->wasm && current->wasm->in_sandbox && rsp < bottom + PAGE_SIZE)
//        trap();
//}
                   ...
                   uint32 heap_size, char *error_buf, uint32 error_buf_size)
{
     ...
     */
    p = mapped_mem = __vmalloc_node_range_reserve(total_size, map_size, 1,
...
#ifdef OS_ENABLE_HW_BOUND_CHECK
    WASMExecEnv *exec_env_tls = current->wasm ? current->wasm->env : NULL;
#endif
                       ...
                       uint32 *ret)
{
    ...
    struct wasm_task_state *saved_state, state;
    unsigned long fs;
    bool res;
    void *base;
    AOTModuleInstance *inst = (AOTModuleInstance *)exec_env->module_inst;

    saved_state = tsk->wasm;
    state.env = exec_env;
    state.in_sandbox = true;
    preempt_disable();
    fs = x86_fsbase_read_cpu();
    base = inst->memories[0]->memory_data;
    x86_fsbase_write_cpu((unsigned long)base);
    preempt_enable();
    if (wasm_save_state(&state)) {
        if (!aot_get_exception(inst))
            aot_set_exception(inst, "wasm instance killed");
        res = false;
        goto out;
    }
    tsk->wasm = &state;
    res = wasm_runtime_invoke_native(exec_env, func_ptr, func_type, signature,
                                     attachment, argv, argc, ret);
out:
    tsk->wasm = saved_state;
    preempt_disable();
    x86_fsbase_write_cpu(fs);
    preempt_enable();
    return res;
}
