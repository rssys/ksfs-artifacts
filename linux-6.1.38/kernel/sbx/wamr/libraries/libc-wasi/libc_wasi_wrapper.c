                    ...
                    uint32 *ro_data_len)
{
    ...
    if (!validate_native_addr(ro_data_len, (uint32)sizeof(uint32))
        || total_size >= UINT32_MAX
        || !validate_native_addr((void *)ri_data, (uint32)total_size))
        return (wasi_errno_t)-1;
    ...
    *ro_data_len = 0;
    err = wasmtime_ssp_sock_recv_from(curfds, sock, iovec_begin, ri_data_len,
                  ...
                  uint32 *so_data_len)
{
    ...
    if (!validate_native_addr(so_data_len, (uint32)sizeof(uint32))
        || total_size >= UINT32_MAX
    ...
    *so_data_len = 0;
    err = wasmtime_ssp_sock_send_to(curfds, addr_pool, sock, ciovec_begin, si_data_len,
...
{
    return wasi_sock_send_to(exec_env, sock, si_data, si_data_len, si_flags, NULL, so_data_len);
}
...
struct file *wasi_get_file(wasm_module_inst_t module_inst, wasi_fd_t fd, int rights)
{
    wasi_ctx_t wasi_ctx = get_wasi_ctx(module_inst);
    struct file *file;
    struct fd_table *curfds = wasi_ctx_get_curfds(module_inst, wasi_ctx);
    __wasi_rights_t wasm_rights = 0;
    if (rights & WASM_FILE_RIGHT_READ)
        wasm_rights |= __WASI_RIGHT_FD_READ;
    if (rights & WASM_FILE_RIGHT_WRITE)
        wasm_rights |= __WASI_RIGHT_FD_WRITE;

    if (fd_object_get(curfds, &file, fd, wasm_rights, 0))
        return NULL;
