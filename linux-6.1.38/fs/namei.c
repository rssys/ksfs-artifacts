...
static int filename_parentat(int dfd, struct filename *name,
			     ...
			     struct qstr *last, int *type, const struct path *root)
{
	int retval;
	struct nameidata nd;
...
static struct dentry *__kern_path_locked(struct filename *name, struct path *path)
{
	...
	error = filename_parentat(AT_FDCWD, name, 0, path, &last, &type, NULL);
	if (error)
		return ERR_PTR(error);
			       ...
			       struct path *root)
{
	...
	error = filename_parentat(dfd, name, reval_flag, path, &last, &type, root);
	if (error)
		return ERR_PTR(error);
		...
		unsigned int dev)
{
	...
	if (error)
		goto out1;
retry:
	dentry = filename_create(dfd, name, &path, lookup_flags, NULL);
...
int do_mkdirat(int dfd, struct filename *name, umode_t mode, struct path *root)
{
	struct dentry *dentry;
	struct path path;
	int error;
...
retry:
	dentry = filename_create(dfd, name, &path, lookup_flags, root);
	error = PTR_ERR(dentry);
	if (IS_ERR(dentry))
		goto out_putname;
...
{
	return do_mkdirat(dfd, getname(pathname), mode, NULL);
}

SYSCALL_DEFINE2(mkdir, const char __user *, pathname, umode_t, mode)
{
	return do_mkdirat(AT_FDCWD, getname(pathname), mode, NULL);
}
...
int do_rmdir(int dfd, struct filename *name, struct path *root)
{
	struct user_namespace *mnt_userns;
	int error;
	struct dentry *dentry;
	...
	unsigned int lookup_flags = 0;
retry:
	error = filename_parentat(dfd, name, lookup_flags, &path, &last, &type, root);
	if (error)
		goto exit1;
...
{
	return do_rmdir(AT_FDCWD, getname(pathname), NULL);
}
 ...
 */
int do_unlinkat(int dfd, struct filename *name, struct path *root)
{
	int error;
	struct dentry *dentry;
	struct path path;
...
SYSCALL_DEFINE3(unlinkat, int, dfd, const char __user *, pathname, int, flag)
{
	...
	if (flag & AT_REMOVEDIR)
		return do_rmdir(dfd, getname(pathname), NULL);
	return do_unlinkat(dfd, getname(pathname), NULL);
}

SYSCALL_DEFINE1(unlink, const char __user *, pathname)
{
	return do_unlinkat(AT_FDCWD, getname(pathname), NULL);
}
...
int do_symlinkat(struct filename *from, int newdfd, struct filename *to, struct path *root)
{
	int error;
	struct dentry *dentry;
	struct path path;
...
retry:
	dentry = filename_create(newdfd, to, &path, lookup_flags, root);
	error = PTR_ERR(dentry);
	if (IS_ERR(dentry))
		goto out_putnames;
...
{
	return do_symlinkat(getname(oldname), newdfd, getname(newname), NULL);
}

SYSCALL_DEFINE2(symlink, const char __user *, oldname, const char __user *, newname)
{
	return do_symlinkat(getname(oldname), AT_FDCWD, getname(newname), NULL);
}
...
int do_linkat(int olddfd, struct filename *old, int newdfd,
	      ...
	      struct path *oldroot, struct path *newroot)
{
	struct user_namespace *mnt_userns;
	...
	if (flags & AT_SYMLINK_FOLLOW)
		how |= LOOKUP_FOLLOW;
retry:
	error = filename_lookup(olddfd, old, how, &old_path, oldroot);
	if (error)
		goto out_putnames;

	new_dentry = filename_create(newdfd, new, &new_path,
					(how & LOOKUP_REVAL), newroot);
		...
		int, newdfd, const char __user *, newname, int, flags)
{
	return do_linkat(olddfd, getname_uflags(oldname, flags),
		newdfd, getname(newname), flags, NULL, NULL);
}

SYSCALL_DEFINE2(link, const char __user *, oldname, const char __user *, newname)
{
	return do_linkat(AT_FDCWD, getname(oldname), AT_FDCWD, getname(newname), 0, NULL, NULL);
}
