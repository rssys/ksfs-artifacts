...
extern struct workqueue_struct *fuse_async_wq;

struct fuse_async_ctx {
...
struct fuse_conn_async {
	int n_shared;
	int n_exclusive;
};

#define FUSE_ASYNC_NBVEC_STACK 8
...
#ifdef CONFIG_FUSE_SBX
	/* wasm_instance pointer */
	struct wasm_instance *wasm_instance;
#endif
};
...
#ifdef CONFIG_FUSE_SBX
	struct fuse_conn_sbx *sbx;
#endif

	struct fuse_conn_async *async;
...
inline static void init_async_ctx(struct fuse_async_ctx *ctx)
{
	INIT_LIST_HEAD(&ctx->reads);
	INIT_LIST_HEAD(&ctx->writes);
	ctx->release_lock = false;
	ctx->read_progress = 0;
}

inline static void locate_in_pages(struct fuse_args_pages *ap,
				   size_t p, size_t *i, unsigned *j)
{
	size_t n;
	if (p < ap->descs[0].length) {
		*i = 0;
		*j = p;
	} else {
		n = p - ap->descs[0].length;
		*i = n / PAGE_SIZE + 1;
		*j = n % PAGE_SIZE;
	}
}
