...
void fuse_conn_put(struct fuse_conn *fc)
{
	if (refcount_dec_and_test(&fc->count)) {
		...
		fuse_sbx_conn_free(fc);
#endif
		fuse_async_conn_free(fc);
		if (fiq->ops->release)
			fiq->ops->release(fiq);
	...
	if (error || arg->major != FUSE_KERNEL_VERSION)
		...
		if (arg->minor >= 6) {
			...
			if (flags & FUSE_SECURITY_CTX)
				fc->init_security = 1;
			if (flags & FUSE_ASYNC_ZEROCOPY)
				if (fuse_async_conn_alloc(fc))
					ok = false;
		} else {
...
void fuse_send_init(struct fuse_mount *fm)
{
	...
	flags =
		...
		FUSE_SECURITY_CTX | FUSE_ASYNC_ZEROCOPY;
#ifdef CONFIG_FUSE_DAX
	if (fm->fc->dax)
...
#ifdef CONFIG_FUSE_SBX
	if (ctx->wasm_instance) {
		err = fuse_sbx_conn_alloc(fc, ctx->wasm_instance);
		if (err) {
			release_wasm_instance(ctx->wasm_instance);
			put_wasm_instance(ctx->wasm_instance);
			ctx->wasm_instance = NULL;
			goto err_free_dax;
		}
	}
#endif
...
static int fuse_fill_super(struct super_block *sb, struct fs_context *fsc)
{
...
#ifdef CONFIG_FUSE_SBX
             && !is_file_wasm_instance(ctx->file)
#endif
	     ) ||
	...
	if (ctx->file->f_op == &fuse_dev_operations)
		...
		ctx->wasm_instance = get_wasm_instance_from_file(ctx->file);
		if (!ctx->wasm_instance)
			return -EINVAL;
		if (!use_wasm_instance(ctx->wasm_instance, true)) {
			put_wasm_instance(ctx->wasm_instance);
			ctx->wasm_instance = NULL;
			return -EINVAL;
		}
